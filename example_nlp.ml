...
let rec seq : ('s,'a) Rule.t list -> ('s,'a list) Rule.t = function
  | [] -> return []
  | x::xs -> x @> seq xs @> return (fun xs x -> x::xs)
...
let bnf : (cat * (tree,tree) Rule.t list) list = [
  NP        , [pronoun];
  NP        , [det;noun];
  VP        , [verb;np];
  VP        , [verb;np;pp];
...
let gram =
  alt (List.map bnf ~f:(fun (lhs,rhs) ->
    seq rhs @> return (fun xs -> Cat (lhs,xs))
  ))
